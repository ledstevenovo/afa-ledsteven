...
def main():
    ...
    ap.add_argument('--n_imgs', type=int, default=N_IMGS)
    ap.add_argument('--seeds', type=str, default=','.join(str(s) for s in SEEDS))
    ap.add_argument('--timesteps', type=str, default=','.join(str(t) for t in TIMESTEPS))
    args = ap.parse_args()
    ...
    t0 = time.time()
    model = Model.load_init(model_paths=[args.expert], st_model_path=args.st_model_file,
    ...
    recs = [json.loads(l) for l in open(args.dataset_json)][:args.n_imgs]
    ...
    torch.save(dict(name=args.name, expert=args.expert, timesteps=timesteps, seeds=seeds,
                    n_imgs=args.n_imgs, combos=combos), dest)
