...
| # | 结论 | 证据 |
|---|---|---|
| 1 | **AFA 在论文的主指标 ImageReward 上显著优于全部三个单专家**，方向与论文一致 | AFA +0.4556 vs 最好单专家 RV +0.3486，配对差 **+0.1070 ± 0.0299（3.6 SE）**，逐 prompt 胜 121 / 负 76 |
...
| 4 | **聚合器本身的净贡献远小于论文暗示的量级**：把"框架/VAE 效应"剥离后只剩 +0.086 IR（边际显著） | `epiCRealism_in_AFA` +0.370 vs `only_epiCRealism` +0.130 → 框架贡献 +0.24；聚合器贡献 +0.086 |
| 5 | **MSE 目标在第 1 个 epoch 就饱和**，后 9 个 epoch（9 万样本）对损失毫无贡献 | eval_loss 0.09178 → 0.09147（−0.34%），随后 9 个 epoch 波动 ±0.00003 |
| 6 | 路由塌成"近似硬选择 + 丢弃一个专家" | `\|attn−0.5\|`=0.484（近上限 0.5）；epiCRealism 权重 **0.022** |
| 7 | **最可能的复现偏差来源是专家版本**：我们的三个单专家彼此差异 5%，论文的三个 base model 只差 0.5% | 我们 CLIP-T 26.43–27.70（5%），论文 0.2566–0.2620（0.5%） |

**判定：部分复现。** 方向正确（AFA ≥ 单专家，且在偏好指标上显著更好），但幅度明显小于论文（论文声称在全部指标上优于所有 base model），且有两项被不同单专家反超。
...
|---|---|---|---|---|
| **IR**（论文主指标） | **1/4** | **+0.107**（3.6 SE，显著） | **+0.086**（1.9 SE，边际显著） | 支持论文主张 |
| **CLIP-I** | **1/4** | +0.006 | – | 语义/逐图保真未退化 |
| **CLIP-T**（DrawBench） | **1/4** | +0.16 | +0.15 | 文本对齐改善 |
...
| 对比 | 差值 ± SE | SE 倍数 | 胜/负 |
|---|---|---|---|
| AFA − Realistic Vision V5.1（最好单专家） | **+0.1070 ± 0.0299** | 3.6 | 121 / 76 |
| AFA − majicMIX v6 | +0.2697 ± 0.0424 | 6.4 | 136 / 60 |
| AFA − epiCRealism | +0.3260 ± 0.0523 | 6.2 | 134 / 65 |
| AFA − epiCRealism in AFA（同 VAE/文本塔） | +0.0857 ± 0.0457 | 1.9 | 106 / 89 |
...
**eval_loss 在 epoch 1 就饱和**：0.09178 → 0.09147（−0.34%），随后 9 个 epoch 在 ±0.00003 内波动。即**后 90% 的训练对 MSE 毫无贡献**——这与本项目 T4 阶段的观察（`RESULTS_T4.md`）在论文配方下完全一致。
...
而聚合器在固定 eval batch 上实际只拿到 **0.45%** 的 MSE 改进（0.09178 → 0.09147）。两者不可直接比较（评测集不同），但差距（4% 上界 vs 0.45% 实际）说明：

```text
MSE 目标下可学空间本来就小（约 4%），而聚合器（软权重、特征级、单遍梯度）只实现了其中一小部分。
```

### 3.4 增益的归因分解（本次复现最有价值的一条）

用 `epiCRealism in AFA`（同框架、同 VAE/文本塔、只跳过聚合器）做对照：

| 环节 | IR 贡献 |
|---|---|
| 单专家自身（epiCRealism 自带 VAE 管线） | +0.130 |
| **换成 AFA 框架（SD1.5 的 VAE + 共享文本塔安排）** | **+0.240** |
| **聚合器的路由本身** | **+0.086**（边际显著） |

结论：**"AFA 优于单专家"里有约 3/4 来自框架/VAE 的差异，而不是聚合器的贡献**。这也解释了为什么 FID 反而略输 RV——聚合器在分布层面没有带来真实感提升。
...
|---|---|---|
| 论文（Table 1） | 0.2566 / 0.2602 / 0.2620 → **0.5%** | 5.4102 / 5.4624 / 5.4881 → **0.08** |
| 本次复现 | 26.43 / 26.71 / 27.70 → **5%** | 5.369 / 5.447 / 5.596 → **0.23** |

我们的三个专家彼此差异比论文的大一个数量级。论文给的是 Civitai **modelVersionId**（ER 134065 / MMR 176425 / RV 130072），本次用的是 HF 上同名文件（`Kalashnikov/epiCRealism`、`digiplay/majicMIX_realistic_v6`、`SG161222/Realistic_Vision_V5.1_noVAE`），**版本对应关系未核验**。若版本不同，"专家多样性过高/过低"会直接影响聚合器的可学空间与增益幅度。

### 4.2 FID 略输 RV 的含义

FID 29.82（AFA）vs 28.96（RV）、31.36（MMR）、45.25（ER）：

- AFA 落在"最好与次好单专家之间"，符合"特征级加权平均 → 输出略被平滑"的预期失效模式；
- 但 CLIP-I（0.700，最高）表明**不是语义漂移**，而是分布层面的多样性/锐度损失；
- 我们的 FID 使用 torchvision InceptionV3 权重（见 §5），绝对值不可与论文对比，但**表内比较有效**。
...
| AFA 在 CLIP-T / CLIP-I 上更好 | 两项均第一 | ✅ 成立 |
| AFA 在 AES 上更好 | 第二，落后 MMR 0.06 | ❌ 未成立 |
| AFA 在 FID 上更好 | 第二，落后 RV 0.86 | ❌ 未成立 |
| 增益来自聚合器 | 主要来自框架/VAE（+0.24 vs +0.086） | ⚠️ 部分成立 |
| 优于静态权重融合方法 | 未检验 | ⬜ 未知 |
